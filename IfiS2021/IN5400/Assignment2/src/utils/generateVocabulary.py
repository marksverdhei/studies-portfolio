...
def generateVocabulary(data_dir, train_records_list, test_records_list):
    ...
    wordCounterSortSub = [
        sum(wordCounterSort[i*subFactor:(i+1)*subFactor]) for i in range(numbOfWordsSub)]
    ...
    if plotFig:
        ...
        plt.ylabel('Word count in millions (bin size=%d)' %
                   subFactor, fontsize=labelSize)
        plt.tick_params(axis='both', which='major', labelsize=tickSize)
    ...
    if plotFig:
        ...
        plt.plot([x_vals[id5+1], x_vals[id5+1]],
                 [0, countsCumRel[id5]], color='r', linestyle='--')
        plt.plot([0, x_vals[id5+1]], [countsCumRel[id5],
                                      countsCumRel[id5]], color='r', linestyle='--')
        plt.plot([x_vals[id10+1], x_vals[id10+1]],
                 [0, countsCumRel[id10]], color='g', linestyle='--')
        plt.plot([0, x_vals[id10+1]], [countsCumRel[id10],
                                       countsCumRel[id10]], color='g', linestyle='--')
        plt.plot([x_vals[id15+1], x_vals[id15+1]],
                 [0, countsCumRel[id15]], color='m', linestyle='--')
        plt.plot([0, x_vals[id15+1]], [countsCumRel[id15],
                                       countsCumRel[id15]], color='m', linestyle='--')
        plt.tick_params(axis='both', which='major', labelsize=tickSize)
    ...
    if not os.path.exists(data_dir):
        ...
        with open(filename, 'wb') as handle:
            pickle.dump(vocabularyDict, handle,
