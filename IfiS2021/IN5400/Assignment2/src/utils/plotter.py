...
class Plotter():
    def __init__(self, modelParam, config):
        ...
        train_line = self.ax[0].plot(
            [], [], color='blue', label='Train', marker='.', linestyle="")
        val_line = self.ax[0].plot(
            [], [], color='red', label='Validation', marker='.', linestyle="")
    ...
    def update_withval(self, current_epoch, loss, valmeasure, mode):
        ...
        if self.bestvalmeasure is None:
            ...
            self.bestvalmeasure = valmeasure
        print('\n\n\ncurrent best val measure and current valmeasure ',
              self.bestvalmeasure, valmeasure)
