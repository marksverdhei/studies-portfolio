...
def validateCaptions(model, modelParam, config, dataLoader):
        ...
        for idx in range(dataDict['numbOfTruncatedSequences']):
            ...
            with torch.no_grad():
                if idx == 0:
                    ...
                    logits, current_hidden_state = model.net(
                        cnn_features, xTokens, is_train, current_hidden_state.detach())
                    predicted_tokens = torch.cat(
                        (predicted_tokens, logits.argmax(dim=2).detach().cpu()), dim=1)
        ...
        for batchInd in range(predicted_tokens.shape[0]):
            ...
            for kk in range(predicted_tokens.shape[1]):
                ...
                if foundEnd == False:
                    ...
                    tokensentence.append('{:d}'.format(
                        predicted_tokens[batchInd, kk].item()))
            ...
            for kk in range(len(dataDict['allcaptionsAsTokens'][batchInd])):
                ...
                seq2 = ['{:d}'.format(s)
                        for s in seq2]  # tokeintegers to string
    ...
    print(
        f'Evaluation results, BLEU-4: {bleu4}, Cider: {cider},  ROUGE: {avg_rouge_dict["rouge"]}, Meteor: {avg_meteor_dict["meteor"]}')
