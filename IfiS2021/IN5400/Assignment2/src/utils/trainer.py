...
class Trainer():
        ...
        for cur_epoch in range(self.model.start_epoch, self.modelParam['numbOfEpochs']):
            for modeSetup in self.modelParam['modeSetups']:
                ...
                if mode == 'train':
                    ...
                    loss = self.run_epoch(
                        mode, self.model, is_train, cur_epoch)
                    valmeasure = loss
                    ...
                    with torch.no_grad():
                        ...
                        resultsdict = validateCaptions(
                            self.model, self.modelParam, self.config, self.dataLoader)
                        # print(resultsdict) #'cider','rouge', 'meteor'

                        valmeasure = resultsdict['meteor']
                        print('validation measure current epoch:', valmeasure)

                    self.plotter.update_withval(
                        cur_epoch, loss, valmeasure, mode)
    ...
    def run_epoch(self, mode, model, is_train, cur_epoch):
        ...
        if self.modelParam['inNotebook']:
            tt = tqdm_notebook(
                self.dataLoader.myDataDicts[mode], desc='', leave=True, mininterval=0.01, file=sys.stdout)
        else:
            # tt = tqdm_notebook(self.dataLoader.myDataDicts[mode], desc='', leave=True, mininterval=0.01,file=sys.stdout)
            tt = tqdm(self.dataLoader.myDataDicts[mode], desc='',
                      leave=True, mininterval=0.01, file=sys.stdout)
        for dataDict in tt:
            ...
            for iter in range(dataDict['numbOfTruncatedSequences']):
                ...
                logits, current_hidden_state = model.net(
                    cnn_features, xTokens,  is_train)
