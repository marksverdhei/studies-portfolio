...
class DataLoaderWrapper():
    def __init__(self, config, modelParam):
        ...
        self.data_dir_train = self.data_dir + \
            'Train2017_'+config['featurepathstub']
        self.data_dir_val = self.data_dir + \
            'Val2017_'+config['featurepathstub']
        ...
        self.myDataDicts = {}
        self.myDataDicts['train'] = DataLoader(myDatasetTrain, batch_size=self.batch_size_train,
                                               shuffle=True, num_workers=modelParam['numbOfCPUThreadsUsed'], collate_fn=myCollate_fn)
        self.myDataDicts['val'] = DataLoader(myDatasetVal, batch_size=self.batch_size_val,
                                             shuffle=True, num_workers=modelParam['numbOfCPUThreadsUsed'], collate_fn=myCollate_fn)
        return
...
class CollateClass:
    ...
    def __call__(self, batch):
        ...
        if len(batch[0]['cnn_features'].shape) == 1:
            outDict['cnn_features'] = torch.tensor(
                np.stack([x['cnn_features'] for x in batch], axis=0))
        else:
            outDict['cnn_features'] = torch.tensor(
                np.stack([x['cnn_features'][:cutoff, :] for x in batch], axis=0))
        ...
        outDict['allcaptionsAsTokens'] = [x['allcaptionsAsTokens']
                                          for x in batch]
    ...
    def getCaptionMatix(self, outDict):
        ...
        divisionCount = int(
            np.ceil((maxSeqLen-1)/self.truncated_backprop_length))
        ...
        yTokens = captionMatix[:, 1:].reshape(
            (batchSize, self.truncated_backprop_length, divisionCount), order='F')
        yWeights = weightMatrix[:, 1:].reshape(
            (batchSize, self.truncated_backprop_length, divisionCount), order='F')

        xTokens = captionMatix[:, :-1].reshape(
            (batchSize, self.truncated_backprop_length, divisionCount), order='F')
        xWeights = weightMatrix[:, :-1].reshape(
            (batchSize, self.truncated_backprop_length, divisionCount), order='F')
...
class Coco_dataclass_cnn_features():
    def __init__(self, data_dir):
        ...
        if not os.path.isdir(data_dir):
            raise FileNotFoundError('cannot find directory', data_dir)
