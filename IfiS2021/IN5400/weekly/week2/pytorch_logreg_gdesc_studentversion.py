...
class logreglayer(nn.Module):
    def __init__(self, dims):
        ...
        self.w = torch.nn.Parameter(data=torch.tensor(np.random.normal(size=(dims, 1))), requires_grad=True)
...
def train_epoch(model,  trainloader,  criterion, device, optimizer):
    ...
    for batch_idx, data in enumerate(trainloader):
        ...
        labels = data[1].to(device).double()
        ...
        w_prime = model.w - lr * model.w.grad
        b_prime = model.bias - lr * model.bias.grad

        with torch.no_grad():
            model.w.copy_(w_prime)
            model.bias.copy_(b_prime)
        # optimizer.step()
...
def run():
    ...
    print(xtr)
    dtr = TensorDataset(torch.tensor(xtr), torch.tensor(ytr))
    # TensorDataset from tensors from xv, yv - our validation features and labels
    dv = TensorDataset(torch.tensor(xv), torch.tensor(yv))
    ...
    model.train()
    # your logreglayer properly initialized
    ...
    dte = TensorDataset(torch.tensor(xt), torch.tensor(yt))
